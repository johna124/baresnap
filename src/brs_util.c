...
void brs_manifest_entry_free(BrsManifestEntry *e)
{
    ...
    /* 1) Liberamos las cadenas de texto dinámicas */
    free(e->path);
    e->path = NULL;
    ...
    /* 2) EL REMEDIO CONTRA EL LEAK 109.6: Liberamos las tablas dinámicas de bloques */
    if (e->chunks) {
        free(e->chunks);
        e->chunks = NULL;
    }
    ...
    if (e->delta_source_chunks) {
        free(e->delta_source_chunks);
        e->delta_source_chunks = NULL;
    }
    e->delta_source_count = 0;
    e->delta_source_cap = 0;
}
