 ...
 * extract
 * ==========================================================================*/
static int extract_match(const char *path, const char *const *paths, size_t n_paths)
{
    for (size_t p = 0; p < n_paths; ++p) {
        size_t plen = strlen(paths[p]);
        if (plen > 0 && paths[p][plen - 1] == '/') plen--;
        if (plen == 0) return 1;
        if (strncmp(path, paths[p], plen) == 0) {
...
static void brs_extract_apply_metadata(const char *path, const BrsManifestEntry *e)
{
    ...
    if (e->type == BRS_FILETYPE_SYMLINK) {
        if (e->uid != 0 || e->gid != 0) (void)lchown(path, (uid_t)e->uid, (gid_t)e->gid);
    ...
    (void)chmod(path, (mode_t)(e->mode & 07777));
    if (e->uid != 0 || e->gid != 0) (void)chown(path, (uid_t)e->uid, (gid_t)e->gid);
    struct timespec ts[2];
    ts[0].tv_sec = (time_t)(e->mtime_ns / 1000000000ULL);
    ts[0].tv_nsec = (long)(e->mtime_ns % 1000000000ULL);
    ts[1] = ts[0];
                     ...
                     _Atomic int *cancel_flag)
{
    ...
    for (uint64_t i = 0; i < snap.entries_len; ++i) {
        ...
        if (e->type == BRS_FILETYPE_FILE) {
            ...
            if (symlink(e->symlink_target ? e->symlink_target : "", full) != 0)
                fprintf(stderr,
                        "warning: cannot create symlink: %s\n",
                        e->path);
    ...
    printf("extracted %llu item(s) to %s\n",
           (unsigned long long)chunks_done, target_real);
                ...
                int long_fmt, int recursive, const char *match)
{
    ...
    if (match && match[0] != '\0') {
        ...
        printf("-- %llu coincidencia(s) para '%s'\n", (unsigned long long)found, match);
        rc = (found > 0) ? 0 : 1;
        goto cleanup;
    }

    {
        ...
        for (size_t pi = 0; pi < n_prefixes; ++pi) {
            ...
            if (plen > 0) {
                ...
                if (!exists) { fprintf(stderr, "ruta no encontrada en el snapshot: %s\n", pfx); continue; }
            }
            if (recursive) {
                for (uint64_t i = 0; i < snap.entries_len; ++i) {
                    const BrsManifestEntry *e = &snap.entries[i];
