...
static int tui_progress_cb(void *user, const char *phase,
...
static void panel_clear(BrsPanel *p)
{
    ...
    for (size_t i = 0; i < p->count; ++i) {
        if (p->items[i]) free(p->items[i]);
        if (p->aux[i]) free(p->aux[i]);
    }
...
static void update_virtual(TuiCtx *ctx, BrsPanel *p)
{
    ...
    char clean_prefix[BRS_PATH_MAX];
    snprintf(clean_prefix, sizeof clean_prefix, "%s", p->current_path);

    /* Quita barras finales duplicadas */
    size_t plen = strlen(clean_prefix);
    while (plen > 1 && clean_prefix[plen - 1] == '/') {
        clean_prefix[plen - 1] = '\0';
        plen--;
    }

    /* prefix_rel es la ruta dentro del snapshot, sin barra inicial */
    const char *prefix_rel = clean_prefix;
    while (*prefix_rel == '/')
        prefix_rel++;

    int es_raiz = (prefix_rel[0] == '\0');
    size_t m_len = strlen(prefix_rel);
    ...
    for (uint64_t i = 0; i < ctx->st.snap.entries_len; ++i) {
        ...
        /* Normaliza el path del snapshot: nunca debe llevar barra inicial */
        const char *e_path = e->path;
        while (*e_path == '/')
            e_path++;
        ...
        if (!es_raiz) {
            if (strncmp(e_path, prefix_rel, m_len) != 0)
                continue;

            if (e_path[m_len] != '\0' && e_path[m_len] != '/')
                continue;
        }

        const char *rest = es_raiz ? e_path : (e_path + m_len);
     ...
     if (slash) {
         ...
         if (es_raiz) {
             ...
             snprintf(aux, sizeof aux, "%s/%.*s",
                      prefix_rel, (int)dlen, rest);
         }
     } else if (e->type == BRS_FILETYPE_DIR) {
         snprintf(disp, sizeof disp, "%s/", rest);
         snprintf(aux, sizeof aux, "%s", e_path);
     } else {
         snprintf(disp, sizeof disp, "%-24s (%llu B)",
                  rest, (unsigned long long)e->size);
         snprintf(aux, sizeof aux, "%s", e_path);
     }
        /* Evitar duplicados por ruta auxiliar, no solo por nombre visible */
        int dup = 0;
        for (size_t d = 0; d < p->count; ++d) {
            if (p->aux[d] && strcmp(p->aux[d], aux) == 0) {
                dup = 1;
                break;
            }
        }
...
static void *worker_main(void *arg)
{
    ...
    fprintf(stderr, "worker_main: start fn=%p arg=%p\n",
            (void *)job->fn, job->arg);
    fflush(stderr);

TLOG("worker_main: start\n");
int rc = job->fn(job->arg);
TLOG("worker_main: done rc=%d\n", rc);

    fprintf(stderr, "worker_main: fn returned rc=%d\n", rc);
    fflush(stderr);
...
static void run_with_progress(const char *title, int (*fn)(void *), void *arg)
{
    ...
    clock_gettime(CLOCK_MONOTONIC, &rate_t);
        ...
        if (win) {
            ...
            /* Forzar el modo no bloqueante absoluto en esta ventana */
nodelay(win, TRUE);

int ch;
while ((ch = wgetch(win)) != ERR) {
    if (ch == 3 || ch == 27) { /* Ctrl+C o ESC */
...
static int extract_fn(void *arg)
{
    ...
    fprintf(stderr,
            "extract_fn: enter repo='%s' snap='%s' target='%s' n=%zu\n",
            j->repo, j->snap, j->target, j->n);
    fflush(stderr);

    for (size_t i = 0; i < j->n; ++i) {
        fprintf(stderr, "extract_fn: path[%zu] = '%s'\n",
                i, j->paths[i]);
    }
    fflush(stderr);

TLOG("extract_fn: before brs_repo_extract\n");    
int rc = brs_repo_extract(j->repo, j->snap,
                              (const char *const *)j->paths, j->n,
                              j->target,
                              tui_progress_cb, NULL,
                              &g_tui_cancel);
TLOG("extract_fn: after brs_repo_extract rc=%d\n", rc);
    fprintf(stderr, "extract_fn: brs_repo_extract rc=%d\n", rc);
    fflush(stderr);
...
typedef struct {
    ...
    int zstd_level;  /* 1-22 */
    int hash_algo;   /* 0=SHA256, 1=Blake2b */
} InitJob;

static int init_fn(void *arg)
{
    InitJob *j = (InitJob *)arg;
    /* Pasamos dinámicamente j->hash_algo en lugar del 1 estático */
    return brs_repo_init(j->repo, j->encrypt, j->cipher_algo, j->compression, j->zstd_level, j->hash_algo);
}
...
return brs_repo_verify(j->repo, 0, tui_progress_cb, NULL, &g_tui_cancel);
...
static int open_snapshot(TuiCtx *ctx, BrsPanel *p, const char *name)
{
    ...
    if (brs_path_join(sp, sizeof sp, snaps, name) != 0)
        return -1;
    ...
    snprintf(ctx->vroot, sizeof ctx->vroot, "/");
...
static void panel_enter(TuiCtx *ctx, BrsPanel *p)
{
    ...
    if (p->mode == PM_REPO_ROOT) {
        ...
        if (ln > 5 && strcmp(item + ln - 5, ".snap") == 0) {
            char snap_name_tmp[256];

            snprintf(snap_name_tmp, sizeof snap_name_tmp, "%s", item);

            if (open_snapshot(ctx, p, snap_name_tmp) != 0)
    ...
    if (ln > 0 && item[ln - 1] == '/' && p->aux[p->selected_index]) {
        char aux_tmp[BRS_PATH_MAX];

        snprintf(aux_tmp, sizeof aux_tmp, "%s", p->aux[p->selected_index]);

        size_t alen = strlen(aux_tmp);

        if (alen > 0 && aux_tmp[alen - 1] == '/')
            snprintf(p->current_path, sizeof p->current_path, "%s", aux_tmp);
        else
            snprintf(p->current_path, sizeof p->current_path, "%s/", aux_tmp);
        ...
        if (p->count <= 1)
            info_popup("Empty folder in snapshot");
    }
}
...
if (ctx->right.mode == PM_REPO_ROOT)
    refresh_panel(ctx, &ctx->right);
...
static void do_extract(TuiCtx *ctx)
{
    ...
    for (size_t i = 0; i < p->count; ++i) {
        if (p->sel[i] && p->aux[i] && p->aux[i][0] != '\0')
            picked++;
    }
    ...
    for (size_t i = 0; i < p->count; ++i) {
        ...
        snprintf(path_buf, sizeof path_buf, "%s", src);
    ...
    if (k == 0) {
        free(j->paths);
        free(j);
        info_popup("No valid items selected for extraction");
        return;
    }

    snprintf(j->repo, sizeof j->repo, "%s", ctx->st.repo);
    memset(j->snap, 0, sizeof j->snap);
    ...
    fprintf(stderr, "TUI extract: repo='%s' snap='%s' target='%s' n=%zu\n",
            j->repo, j->snap, j->target, j->n);

    for (size_t i = 0; i < j->n; ++i) {
        fprintf(stderr, "TUI extract path[%zu] = '%s'\n",
                i, j->paths[i]);
    }
...
static void do_init_repo(TuiCtx *ctx)
{
    ...
    /* === AÑADE EL MENÚ AQUÍ === */
    static const char *hash_items[] = {"SHA256 (default)", "Blake2b (faster)"};
    int hash_choice = pick_from_list("Hash algorithm", (char **)hash_items, 2);
    if (hash_choice < 0) return;
    /* ========================== */
