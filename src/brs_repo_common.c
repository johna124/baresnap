...
int derive_repo_key(const BrsRepoConfig *cfg, BrsSecureKey *key)
{
    ...
    // 1. COMPROBACIÓN ANTI-BLOQUEO SSH: Intentamos leer primero la variable de entorno
    const char *env_pass = getenv("BARESNAP_PASSPHRASE");
    
    if (env_pass && env_pass[0] != '\0') {
        // Si la variable existe, la copiamos de forma segura al buffer de procesamiento
        strncpy(pass, env_pass, BRS_PASSPHRASE_MAX);
        pass[BRS_PASSPHRASE_MAX] = '\0';
    } else {
        // 2. Si no hay variable de entorno, saltamos al modo interactivo por teclado original
        if (brs_read_passphrase(pass, sizeof pass, "Passphrase: ") != 0) {
            return -1;
        }
    }

    // Derivamos la clave criptográfica usando la contraseña recuperada
    int rc = brs_derive_key(cfg, pass, key);
    
    // Borramos de forma segura la contraseña de la memoria RAM
...
/* Global persistent variables for chunk stream reuse */
static __thread uint64_t g_cached_pack_id = 0;
static __thread BrsVfsFile *g_cached_pack_file = NULL;
static __thread BrsVfs *g_cached_vfs_ctx = NULL;

/* Safe cleanup wrapper to tear down the persistent block handle */
void brs_clear_cached_pack_handle(void)
{
    if (g_cached_pack_file != NULL) {
        brs_vfs_fclose(g_cached_pack_file);
        g_cached_pack_file = NULL;
    }
    g_cached_pack_id = 0;
    g_cached_vfs_ctx = NULL;
}

/* Internal persistent sequential chunk block scanner */
static int read_chunk_streaming_cached(BrsVfsFile *f, const BrsChunkLocation *loc, BrsBuffer *out)
{
...
/* Your optimized cache persistent entry point */
int read_chunk_from_pack(const char *repo, const BrsChunkLocation *loc, BrsBuffer *out)
{
    ...
    if (g_cached_pack_file != NULL && g_cached_vfs_ctx != vfs) {
        brs_clear_cached_pack_handle();
    }

    if (g_cached_pack_file == NULL || g_cached_pack_id != loc->pack_id) {
        brs_clear_cached_pack_handle();

        char name[40], rel[BRS_PATH_MAX];
        snprintf(name, sizeof name, "%llu.pack", (unsigned long long)loc->pack_id);
        snprintf(rel, sizeof rel, "packs/%s", name);

        g_cached_pack_file = brs_vfs_fopen(vfs, rel, BRS_VFS_OPEN_READ);
        if (!g_cached_pack_file) {
            if (owns_vfs) brs_vfs_close(vfs);
            return -1;
        }
        g_cached_pack_id = loc->pack_id;
        g_cached_vfs_ctx = vfs;
    }

    int rc = read_chunk_streaming_cached(g_cached_pack_file, loc, out);
