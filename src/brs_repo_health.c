...
int brs_health_repair(const char *repo_path, BrsHealthReport *report)
{
    ...
    /* FIX C5: recuperar config perdido preservando UUID original */
        /* FIX F1: detectar snapshots cifrados ANTES de recrear la config.
     * Recrear una config plana sobre un repo con packs/snapshots cifrados
     * provoca corrupción silenciosa: las operaciones posteriores escribirían
     * e interpretarían metadatos bajo supuestos criptográficos erróneos. */
    if (!health_config_loadable(repo_path)) {
        ...
        if (brs_repo_init(repo_path, 0, 0, 0, 3) != 0) {
            fprintf(stderr,
                   "  [REPAIR] ERROR: cannot recreate missing config\n");
            errors++;
        } else {
            g_health_missing_config = 0;
            if (have_uuid || was_encrypted) {
                ...
                if (brs_load_config(repo_path, &cfg) == 0) {
                    ...
                    if (brs_write_config(repo_path, &cfg) != 0) {
                        ...
                        fprintf(stderr,
                               "  [REPAIR] NOTE: the original KDF salt lived in the lost config.\n"
                               "            Encrypted snapshots are only readable if the original\n"
                               "            passphrase is still derivable against this repo.\n");
                    }
                } else {
                    fprintf(stderr,
                           "  [REPAIR] ERROR: cannot reload recreated config\n");
                    errors++;
                }
            }
        }
    ...
    for (size_t i = 0; i < report->orphan_idx_count; ++i) {
        ...
        snprintf(name, sizeof name,"%llu.idx",
                 (unsigned long long)report->orphan_idx[i]);
    ...
    /* ================================================================
    Reconstruir chunks faltantes desde TODOS los packs
    ================================================================= */
    {
        BrsIndexMap full_index;
        if (brs_index_map_init(&full_index, 1024) == 0) {
            ...
            if (brs_index_map_init(&missing_index, 1024) == 0) {
                ...
                if (missing_count > 0) {
                    ...
                    if (brs_write_index_segment(repo_path,
                                                new_seg_id,
                                                &missing_index) == 0) {
                        fprintf(stderr,
    ...
    if (report->corrupt_snap_count > 0) {
        ...
        if (brs_path_join(snaps_dir_rv, sizeof snaps_dir_rv,
            ...
            if (brs_index_map_init(&updated_idx, 1024) == 0) {
                ...
                int rv_has_cfg =
                    (brs_load_config(repo_path, &rv_cfg) == 0);
...
for (size_t i = 0; i < report->corrupt_snap_count; ++i) {
    ...
    int use_key =
        (rv_has_cfg && rv_cfg.encrypted && rv_key_valid);

    if (brs_parse_snapshot(sd.data, sd.size, &ps,
                           use_key ? &rv_key : NULL, (rv_has_cfg ? rv_cfg.cipher_algo : BRS_CIPHER_CHACHA20_POLY1305)) != 0) {
        /* FIX P4: compactacion segura.
         * NO liberar corrupt_snaps[still] porque ese slot
         * ya fue movido en una iteracion previa. */
        if (still != i) {
    ...
    for (uint64_t e = 0; e < ps.entries_len && ok; ++e) {
        ...
        for (uint32_t c = 0; c < se->chunk_count; ++c) {
            if (!brs_index_map_get(&updated_idx, &se->chunks[c])) {
                ok = 0;
                break;
            }
        }
...
report->corrupt_snap_count = still;
    ...
    if (report->corrupt_snap_count > 0) {
        ...
        if (brs_path_join(snaps_dir, sizeof snaps_dir,
            ...
            for (size_t i = 0; i < report->corrupt_snap_count; ++i) {
                ...
                if (rename(src, dst) == 0) {
                    fprintf(stderr,
                           "  [REPAIR] moved to damaged/: %s\n",
                            report->corrupt_snaps[i]);
                    moved++;
                } else {
                    fprintf(stderr,
                           "  [REPAIR] ERROR moving %s: %s\n",
                            report->corrupt_snaps[i],
                            strerror(errno));
                    errors++;
                }
    ...
    if (errors == 0) {
        ...
        fprintf(stderr,
               "  [REPAIR] %d error(s) during repair\n",
                errors);
    }
    return errors == 0 ? 0 : -1;
}
