...
static size_t ssh_read_window(void)
{
    size_t v = ssh_io_from_env(
        "BRS_SSH_READ_WINDOW",
        BRS_SSH_READ_WINDOW_DEFAULT
    );
...
static void ssh_invalidate_read_buffer(BrsSshFileHandle *h)
{
    if (!h)
        return;
...
static ssize_t ssh_read_window_fetch(
    BrsSshFileHandle *h,
    uint64_t off,
    size_t want
)
{
    if (!h || !h->ssh || !h->ssh->remote || want == 0)
        return -1;
    ...
    if (h->rbuf_cap < want) {
        ...
        uint8_t *nb = (uint8_t *)realloc(h->rbuf, ncap);
        if (!nb)
            return -1;
...
static void ssh_free_read_buffer(BrsSshFileHandle *h)
{
    if (!h)
        return;
...
if (pthread_mutexattr_init(&attr) == 0) {
    /* Forzamos el tipo recursivo para soportar reentrada asíncrona limpia */
    pthread_mutexattr_settype(&attr, PTHREAD_MUTEX_RECURSIVE);
    pthread_mutex_init(&ssh->mtx, &attr);
    pthread_mutexattr_destroy(&attr);
} else {
    /* Fallback de emergencia si la pila del sistema operativo está degradada */
...
ssize_t brs_vfs_ssh_fread(BrsVfsFile *f, void *buf, size_t n, uint64_t off)
{
    ...
    /*
     * 1) Intentar servir desde la ventana cacheada.
     */
    if (h->rbuf_len > 0 && off >= h->rbuf_off) {
        uint64_t rel = off - h->rbuf_off;

        if (rel < h->rbuf_len &&
            n <= (h->rbuf_len - (size_t)rel)) {
            memcpy(buf, h->rbuf + rel, n);

            if (g_vfs_read_progress_cb) {
                g_vfs_read_progress_cb(
                    g_vfs_read_progress_ctx,
                    (uint64_t)n,
                    (uint64_t)n
                );
            }
    ...
    /*
     * 2) Si la petición es pequeña, leer una ventana mayor.
     */
    size_t window = ssh_read_window();
    ...
    if (window > 0 && n < window) {
        ...
        ssize_t got = ssh_read_window_fetch(h, off, window);
        ...
        size_t serve = ((size_t)got < n) ? (size_t)got : n;
        memcpy(buf, h->rbuf, serve);
        ...
        return (ssize_t)serve;
    }

    /*
     * 3) Lectura directa clásica para peticiones grandes.
     */
    if (g_vfs_read_progress_cb) {
        g_vfs_read_progress_cb(
            g_vfs_read_progress_ctx,
...
int brs_vfs_sync_index_bulk(BrsVfs *vfs, const char *repo_path, BrsIndexMap *out_map)
{
    ...
    /* 🔒 MUTEX ESTÁTICO DE ENTORNO AISLADO
     * Previene colisiones de TSan y falsos positivos si la memoria del heap
     * está enmascarada o mapeada con estructuras anidadas del Snapshot. */
    static pthread_mutex_t tui_index_mtx = PTHREAD_MUTEX_INITIALIZER;

    if (!vfs || !out_map)
        return -1;

    /* 🛡️ CORTAFUEGOS DE SEGURIDAD CONTRA ENMASCARAMIENTO DE PUNTEROS */
    if (brs_vfs_ssh_backend_name(vfs) == NULL) {
        return -1;
    }
    ...
    /* Usamos el cerrojo global e inmune a alias de memoria de TSan */
    pthread_mutex_lock(&tui_index_mtx);
    int rc = brs_remote_sync_index(ssh->remote, &dump, &dump_len);
    pthread_mutex_unlock(&tui_index_mtx);
