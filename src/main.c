...
{
    if (g_vfs) {
        extern BrsVfs *brs_vfs_context_get(void);
...
static void print_usage(void)
{
    printf(
        ...
        "\n"
        "  # List / search inside a snapshot\n"
        "  baresnap ls /mnt/backup/repo snap_2026.snap -l -r\n"
        "  baresnap ls /mnt/backup/repo snap_2026.snap -m 'some text'\n\n"  
        "  # Search across all snapshots\n"
        "  baresnap search /mnt/backup/repo -m potato\n"
        "  baresnap search ssh://user@host/repo -m '*.conf'\n"
        "\n"
        "  # Extract using wildcard filters\n"
        "  baresnap extract /repo snap.snap /tmp/out -m '*potato*'\n"
        "  baresnap extract /repo snap.snap /tmp/out -m '*.pdf' -m 'docs/*'\n"
        "\n"   
...
static void print_help_extract(void)
{
    printf(
            ...
            "\n"
            "USAGE:\n"
            "  baresnap extract <repo> <snapshot> <target> [-m pattern ...] [paths...]\n"
            "\n"
            "ARGUMENTS:\n"
            ...
            "  [paths...]       Optional filters. Same semantics as -m patterns.\n"
            "\n"
            "FILTER PATTERNS:\n"
            "  If no filter is given, the full snapshot is extracted.\n"
            "\n"
            "  Multiple filters are allowed and are combined with OR logic.\n"
            "\n"
            "  A pattern containing '*', '?' or '[' is interpreted as a glob.\n"
            "  In glob mode, '*' matches any string, including '/'.\n"
            "\n"
            "  A literal pattern without wildcard characters matches either:\n"
            "    1. An exact snapshot path.\n"
            "    2. A directory prefix, extracting everything below it.\n"
            "\n"
            "OPTIONS:\n"
            "  -m, --match PATTERN  Add a filter pattern. Can be repeated.\n"
            "      --timeout <ms>   SSH timeout per operation (default: 60000)\n"
            "\n"
            "IMPORTANT:\n"
            "  Patterns and paths are relative to the snapshot root.\n"
            "  Quote patterns and paths containing spaces.\n"
            "\n"
            "EXAMPLES:\n"
            "  # Extract the full snapshot\n"
            "  baresnap extract /mnt/backup/repo snap_2026.snap /tmp/restored\n"
            "\n"
            "  # Extract by exact path or directory prefix\n"
            "  baresnap extract /repo snap.snap /tmp/out docs/project\n"
            "\n"
            "  # Extract using wildcards\n"
            "  baresnap extract /repo snap.snap /tmp/out -m '*potato*'\n"
            "  baresnap extract /repo snap.snap /tmp/out -m 'potato*'\n"
            "  baresnap extract /repo snap.snap /tmp/out -m '*.pdf'\n"
            "\n"
            "  # Extract multiple patterns\n"
            "  baresnap extract /repo snap.snap /tmp/out -m '*.md' -m 'config/*'\n"
            "\n"
            "  # Extract a file with spaces\n"
            "  baresnap extract ssh://user@host/repo snap.snap /target/ \\\n"
            "      'folder with spaces/file.txt'\n"
            "\n"
            "  # Remote extraction with long timeout\n"
            "  baresnap extract --timeout 180000 ssh://user@host/repo snap.snap /tmp/out \\\n"
            "      -m '*.txt'\n"
            "\n"
            "RECOMMENDED WORKFLOW:\n"
            "  1. Locate the file with search or ls:\n"
            "       baresnap search <repo> -m keyword\n"
            "       baresnap ls <repo> <snapshot> -m keyword\n"
            "\n"
            "  2. Extract using the exact path or a wildcard pattern:\n"
            "       baresnap extract <repo> <snapshot> <target> 'exact/path.txt'\n"
            "       baresnap extract <repo> <snapshot> <target> -m '*keyword*'\n"
            );
        }
...
static void print_help_init(void)
{
    printf(
        ...
        "              [--zstd-level N] [--hash fnv1a|blake2b]\n"
        "\n"
        "OPTIONS:\n"
...
{
    printf(
        "baresnap verify [--fast] - Verify repository integrity\n"
        "\n"
        "USAGE:\n"
...
static void print_help_ls(void)
{
    printf(
        "baresnap ls - List and search files inside a snapshot\n"
        "\n"
        "USAGE:\n"
        "  baresnap ls <repo> <snapshot> [prefix] [-l] [-r] [-m text]\n"
        "\n"
        "ARGUMENTS:\n"
        "  <repo>       Repository path (local or ssh://user@host/path)\n"
        "  <snapshot>   Snapshot name (or prefix) or full path to .snap\n"
        "  [prefix]     Optional prefix/directory inside the snapshot\n"
        "\n"
        "OPTIONS:\n"
        "  -l, --long       Long format: type, size, date and path\n"
        "  -r, --recursive  Recursive listing under the prefix\n"
        "  -m, --match TXT  Case-insensitive substring search in path\n"
        "  --timeout <ms>   SSH timeout (default: 60000)\n"
        "\n"
        "EXAMPLES:\n"
        "  # 1. Find a file over SSH using recursive list and grep\n"
        "  ./baresnap ls ssh://test@192.168.1.1/home/test/baresnapblake \\\n"
        "      2026-09-13_17-53-21-000_c317786f.snap -r | grep zlibWrapper\n"
        "\n"
        "  # 2. Extract the exact path found in the previous step\n"
        "  ./baresnap extract ssh://test@192.168.1.2/home/test/baresnapblake \\\n"
        "      2026-09-13_17-53-21-000_c317786f.snap /home/mint/ \\\n"
        "      baresnap/third_party/zstd/zlibWrapper\n"
        "\n"
        "  # 3. MODERN ALTERNATIVE: Use --match (-m) instead of grep (faster over SSH)\n"
        "  ./baresnap ls ssh://test@192.168.1.1/home/test/baresnapblake \\\n"
        "      2026-09-13_17-53-21-000_c317786f.snap -m zlibWrapper\n"
        "\n"
        "  # 4. Extracting files with weird spaces (use single quotes)\n"
        "  ./baresnap extract ssh://user@host/repo snap.snap /target/ \\\n"
        "      'baresnap_v2_3_9_1/test new/new potato  more faster txt'\n"
    );
}

static void print_help_search(void)
{
    printf(
            "baresnap search - Search file paths across all snapshots\n"
            "\n"
            "USAGE:\n"
            "  baresnap search <repo> -m <pattern> [-s snapshot] [--glob]\n"
            "\n"
            "ARGUMENTS:\n"
            "  <repo>           Repository path (local or ssh://user@host/path)\n"
            "\n"
            "OPTIONS:\n"
            "  -m, --match TXT      Search pattern.\n"
            "                       If the pattern does not contain '*', '?' or '[',\n"
            "                       it is treated as a case-insensitive substring.\n"
            "                       If it contains wildcard characters, it is treated\n"
            "                       as a glob pattern automatically.\n"
            "  -s, --snapshot SNAP  Search only inside one snapshot.\n"
            "                       If omitted, all snapshots are searched.\n"
            "      --glob           Force glob matching even if automatic detection\n"
            "                       is not desired.\n"
            "      --timeout <ms>   SSH timeout per operation (default: 60000)\n"
            "\n"
            "GLOB BEHAVIOR:\n"
            ...
            "\n"
            "  Quote patterns to prevent the shell from expanding them.\n"
            "\n"
            "EXAMPLES:\n"
            "  # Case-insensitive substring search across all snapshots\n"
            "  baresnap search /mnt/backup/repo -m potato\n"
            "\n"
            "  # Search only inside a specific snapshot\n"
            "  baresnap search /mnt/backup/repo -s 2026-09-16_14-01-41.snap -m potato\n"
            "\n"
            "  # Glob search across all snapshots\n"
            "  baresnap search ssh://user@host/repo -m '*.conf'\n"
            "\n"
            "  # Find anything containing 'potato' in the path\n"
            "  baresnap search ssh://user@host/repo -m '*potato*'\n"
            "\n"
            "  # Find all PDFs under any directory\n"
            "  baresnap search ssh://user@host/repo -m '*.pdf'\n"
            "\n"
            "  # Remote search with custom timeout\n"
            "  baresnap search --timeout 90000 ssh://user@host/repo -m 'config*'\n"
            );
          }
...
int main(int argc, char **argv)
{
    ...
    if (strcmp(command, "health") == 0) {
     ...
     if (argc < 3) {
         fprintf(stderr, "usage: baresnap init <repo> [--encrypt] [--compression lz4|zstd] [--zstd-level N] [--hash fnv1a|blake2b]\n");
         return 1;
     }
     const char *repo = argv[2];
     int encrypt = 0;
     ...
     int compression = 0;
     int zstd_level = 3;
     int hash_algo = 0;  /* 0 = FNV1A (default), 1 = BLAKE2B */
     ...
     for (int i = 3; i < argc; ++i) {
         if (strcmp(argv[i], "--encrypt") == 0) {
            ...
            ++i;
            if (strcmp(argv[i], "blake2b") == 0) {
                hash_algo = BRS_HASH_BLAKE2B_128;
            } else if (strcmp(argv[i], "fnv1a") == 0) {
                hash_algo = BRS_HASH_FNV1A_128;
            } else {
                 fprintf(stderr, "error: unknown hash algorithm '%s' (use 'fnv1a' or 'blake2b')\n", argv[i]);
                 return 1;
             }
         } else {
     ...
     /* LLAMADA ACTUALIZADA CON 6 ARGUMENTOS */
     int rc = brs_repo_init(repo, encrypt, cipher_algo, compression, zstd_level, hash_algo);
    ...
    if (strcmp(command, "create") == 0) {
        ...
        if (!brs_path_exists(config_path)) {
            ...
            fprintf(stderr, "Initializing new repository...\n");
             int rc_init = brs_repo_init(repo, 0, 0, compression < 0 ? 0 : compression, zstd_level, 0); /* 0 = FNV1A default */
            if (rc_init != 0) {
    ...
    if (strcmp(command, "restore") == 0) {
        ...
        /* This handles the full extended help window (--help) automatically */
        check_help_in_args(argc, argv, 2, print_help_verify);
        if (argc < 3) {
            fprintf(stderr, "usage: baresnap verify <repo> [--fast]\n");
            return 1;
        }
        
        /* Detect if the --fast flag is present as the 4th argument */
        int fast_mode = 0;
        ...
        /* Pass our brand-new fast_mode integer directly down to the updated engine */
        int rc = brs_repo_verify(argv[2], fast_mode, cli_progress_callback, NULL, &g_cli_cancel);
    ...
    if (strcmp(command, "ls") == 0) {
        check_help_in_args(argc, argv, 2, print_help_ls);
        if (argc < 4) {
    ...
    check_help_in_args(argc, argv, 2, print_help_search);

    const char *repo = NULL;
    const char *pattern = NULL;
    const char *snap_filter = NULL;
    int use_glob = 0;
    ...
    for (int i = 2; i < argc; ++i) {
        if ((strcmp(argv[i], "-m") == 0 ||
            ...
            fprintf(stderr, "error: unknown search option: '%s'\n", argv[i]);
            return 1;
        }
    }
    ...
    if (cli_repo_open(repo) != 0)
        return 1;

    cli_progress_init(&g_prog);
    int rc = brs_repo_search(repo, snap_filter, pattern, use_glob,
                             cli_progress_callback, NULL, &g_cli_cancel);
    cli_progress_finish(&g_prog);
    cli_repo_close();
    ...
    if (argc < 5) {
        fprintf(stderr,
                "usage: baresnap extract <repo> <snapshot> <target> "
                "[-m pattern ...] [paths...]\n");
        return 1;
    }
    ...
    const char *patterns[1024];
    size_t n_patterns = 0;

    int no_more_options = 0;

    for (int i = 5; i < argc; ++i) {
        if (!no_more_options && strcmp(argv[i], "--") == 0) {
            no_more_options = 1;
            continue;
        }
        ...
        if (!no_more_options && argv[i][0] == '-') {
            fprintf(stderr, "error: unknown extract option: '%s'\n", argv[i]);
            return 1;
        }
    ...
    int rc = brs_repo_extract(repo, snap,
                              n_patterns ? patterns : NULL,
                              n_patterns,
                              target,
                              cli_progress_callback, NULL,
                              &g_cli_cancel);
