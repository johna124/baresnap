...
echo "⚠️  WARNING: strip is not installed. The binary will not be structurally optimized."
fi
if [ "$FALTAN" -eq 1 ]; then
echo ""
...
OPT_DEBUG="-O0 -g -ggdb3 -fno-omit-frame-pointer -fno-inline"
CFLAGS="-std=c11 -Wall -Wextra $OPT_DEBUG -ffunction-sections -fdata-sections -fno-ident -fno-asynchronous-unwind-tables $MAP_FLAGS -Isrc -Ithird_party/monocypher -Ithird_party/xdelta/xdelta3 -Ithird_party/aes-gcm -DHAVE_CONFIG_H=0 -DXD3_USE_LARGESIZET=1 -DXD3_MAIN=0 -DXD3_DEBUG=0 -DREGRESSION_TEST=0 -DSECONDARY_DJW=0 -DSECONDARY_FGK=0 -DSECONDARY_LZMA=0 -DEXTERNAL_COMPRESSION=0 -DVCDIFF_TOOLS=0 -DSIZEOF_SIZE_T=8 -DSIZEOF_UNSIGNED_INT=4 -DSIZEOF_UNSIGNED_LONG=8 -DSIZEOF_UNSIGNED_LONG_LONG=8 -Dusize_t=uint64_t -Dxoff_t=uint64_t"
ABS="$(pwd)/build"
STAMP="$ABS/.ncurses_ok"

mkdir -p build
# ---- 1. static ncurses 6.5 compiled with musl-gcc ----
if [ ! -f "$STAMP" ]; then
cd third_party/ncurses-6.5
./configure CC="$CC"  CFLAGS="$OPT_DEBUG -fPIC -fno-ident $MAP_FLAGS" --prefix="$ABS/staging" \
...
$CC -std=c11 $OPT_DEBUG -w -fno-ident $MAP_FLAGS -c third_party/monocypher/monocypher.c -o build/monocypher.o

# ---- 2.5 aes-gcm with AES-NI ----
echo "Compiling AES-256-GCM (with AES-NI + PCLMULQDQ)..."
$CC -std=c11 $OPT_DEBUG -w $ARCH_FLAGS -Ithird_party/aes-gcm -c third_party/aes-gcm/aes_gcm.c -o build/aes_gcm.o
...
echo "Compiling zstd..."
cd third_party/zstd
make -C lib CC="$CC" CFLAGS="$OPT_DEBUG -fPIC -fno-ident $MAP_FLAGS" libzstd.a -j"$(nproc)"
...
$CC -std=c11 $OPT_DEBUG -Wall -Wextra -ffunction-sections -fdata-sections -fno-ident -fno-asynchronous-unwind-tables $MAP_FLAGS -Isrc \
