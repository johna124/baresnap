# 🏛️ BareSnap v2.4.0 — The Spartan Manifesto (Hardware-Accelerated Edition)
...
The engine features a self-contained **Selectable-at-Init Encryption** module supporting **XChaCha20-Poly1305** or **AES-256-GCM** with dynamic runtime dispatching.

1. **Hardware Mode (Turbo Silicon)**: On modern x86_64 or ARM64 silicon with crypto-extensions, code uses vector intrinsic instructions (`_mm_aesenc_si128` and hardware Karatsuba multiplication via `PCLMULQDQ`) for maximum throughput [2.1].

2. **Software Mode (Spartan Fallback)**: On legacy hardware lacking AES-NI, the `CPUID` runtime constructor safely reroutes operations to software loops or **XChaCha20-Poly1305** without illegal instruction faults (`SIGILL`).
...
==525052== LEAK SUMMARY: definitely lost: 0 bytes in 0 blocks.
==525052== ERROR SUMMARY: 0 errors from 0 contexts (suppressed: 0 from 0).
```
...
* **Time-Travel 1.2 CLI**: The inotify daemon (~194 KB) that watched and saved the lines of code of this very project with xdelta3 deltas every single time the AI hallucinated.

* **txt2pdf 1.6**: The distiller that converts plain text into 169-page technical manuals without bloating a single byte.
